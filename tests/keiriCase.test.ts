...
/**
 * 最初の画面に「触る」と「申し込む」の2つが、どちらも全部見えていること（kp141）。
 *
 * kp133 で近道のリンクを足したが、縦に積んでいたため、スマホ（幅390px・高さ844px）の
 * 本番で実測すると「もうお決まりの方は、お申し込みへ進む」は 817px＝画面のいちばん下の
 * 端にかかり、全部は見えていなかった（2026-09-24 実測）。
 * 8軒への1通を受け取った店主が最初に見るのはこの1画面なので、
 * 「もう決めた」人がその場で申し込める形にして、それを戻さないように固定する。
 *
 * ★ここで守るのは並べ方だけ。値段・商品・売り文句は じゅんの領分なので触らない。
 */
test("紹介ページの一番上で、触るボタンと申し込むボタンが横に並んでいる（kp141）", () => {
  const src = fs.readFileSync(
    path.join(process.cwd(), "app", "keiri", "case", "page.tsx"),
    "utf8",
  );
  ...
  // 2つのボタンが1つの行（2列の並び）に入っていること。
  // 縦積みに戻すと、申し込むボタンが画面の下にはみ出す。
  assert.ok(
    hero.includes("grid grid-cols-2"),
    "触るボタンと申し込むボタンを、横に2つ並べた形に戻してください（縦に積むと申し込むボタンが1画面目からはみ出します）",
  );
  // どちらも「画面いっぱいに広げる」形に戻していないこと（戻すと縦に積まれる）
  assert.ok(
    !/href="\/keiri\/(demo|apply)"[\s\S]{0,200}?block w-full/.test(hero),
    "一番上の2つのボタンを、横幅いっぱい（block w-full）に戻さないでください",
  );
  // 申し込み側の名前は、下の申し込み枠と同じ「申し込む」にそろえる
  assert.ok(
    /申し込む\s*\n/.test(hero) || hero.includes(">\n              申し込む"),
    "一番上の申し込みボタンの名前は「申し込む」にそろえてください",
  );
  // 主役は今までどおり「まず触ってみる」（塗りのボタン）
  assert.ok(
    hero.includes("bg-amber-500"),
    "主役（まず触ってみる）は塗りのボタンのままにしてください",
  );
  // 本番の /keiri/apply と同じ言い方であること
  assert.ok(
    hero.includes("この画面ではお支払いは発生しません。"),
