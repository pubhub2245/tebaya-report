...
/**
 * トップのボタンの枠（TebayaOnlyBlock）は、**どのお店か分かる前は中身を出す**。
 *
 * 逆にすると、手羽屋が毎日開くトップから、開いた直後だけボタンが消える
 * （2026-09-24 に本番で実際にそうなったので直した）。
 * この枠に入れてよいのはボタンとその名前だけで、金額の出る枠は
 * useIsTebaya() を直に使い、分かる前は出さないままにしてある（別のテストで固定）。
 */
test("トップのボタンの枠は、どのお店か分かる前も手羽屋の画面から消えない", () => {
  const src = readFileSync("app/components/TebayaOnlyGate.tsx", "utf8");
  assert.ok(
    /if\s*\(!checking\s*&&\s*!isTebaya\)\s*return null;/.test(src),
    "TebayaOnlyBlock が、どのお店か分かる前に中身を消す形になっています",
  );
  // ページ全体の門のほうは、これまでどおり分かる前は中身を出さない
  assert.ok(
    src.includes("if (checking) {"),
    "TebayaOnlyGate（ページ全体の門）が、分かる前に中身を出さない形でなくなっています",
  );
});

// ---------------------------------------------------------------------------
// kp122（2026-09-24）：日報に残っていた「手羽屋のもの」2つ
// ---------------------------------------------------------------------------
// ① 「🍗 手羽屋 / 🍖 もも屋」の2択 … 手羽屋だけの屋号なので、
//    経理パッケージを申し込んだお店には意味が無い（選んでも何も変わらない）。
//    出さないときは既定の「手羽屋」のまま＝そのお店の通常の日報として動く
//    （商品マスタ連動・場代の自動入力は、どちらもこの値で働く）。
// ② 「📊 中間報告」のリンク … /interim は棚に「どの店か」の欄がまだ無く、
//    手羽屋以外には開かない画面。押しても門に当たるだけなので、出さない。
//
// ★ 手羽屋は印が空（scope が null）なので、どちらもこれまでどおり出る。

test("屋号の2択（手羽屋/もも屋）は、よそのお店の日報に出さない", () => {
  const src = readFileSync("app/report/page.tsx", "utf8");
  assert.ok(
    /\{showShopPicker && \(/.test(src),
    "屋号の2択が showShopPicker で囲われていません（よそのお店にも出ます）",
  );
  assert.ok(
    /showShopPicker=\{!scope\}/.test(src),
    "showShopPicker に !scope が渡されていません",
  );
});

test("確認画面の「お店」の行も、よそのお店には出さない", () => {
  const src = readFileSync("app/report/page.tsx", "utf8");
  // 出すと、選んでいないのに必ず「🍗 手羽屋」と表示されてしまうため
  assert.ok(
    /\{showShop && \(\s*<Row k="お店"/.test(src),
    "確認画面の「お店」の行が showShop で囲われていません",
  );
  assert.ok(
    /showShop=\{!scope\}/.test(src),
    "showShop に !scope が渡されていません",
  );
});

test("「📊 中間報告」のリンクは、よそのお店の日報に出さない", () => {
  const src = readFileSync("app/report/page.tsx", "utf8");
  assert.ok(
    /\{!scope && \(\s*<a\s+href="\/interim"/.test(src),
    "中間報告のリンクが !scope で囲われていません（押すと門に当たります）",
  );
  // 中間報告の棚（interim_reports）に「どの店か」の欄が付いて門が外れたら、
  // このリンクの囲いも一緒に外してよい。それまでは出さない。
  const gate = readFileSync("app/interim/page.tsx", "utf8");
  assert.ok(
    gate.includes("TebayaOnlyGate"),
    "中間報告の画面から門が外れています。リンクの囲いも見直してください",
  );
});
