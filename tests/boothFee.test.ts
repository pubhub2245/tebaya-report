...
const MASTER = [
  ...
  { name: "PASIO鷹尾", booth_fee_type: "fixed", booth_fee_rate: null, booth_fee_amount: 2200 },
...
test("高城（たかじょう）と鷹尾（たかお）を混ぜない／ながやまとPASIOも混ぜない", () => {
  const rules = buildBoothFeeRules([
    ...MASTER,
    { name: "PASIO高城", booth_fee_type: "fixed", booth_fee_rate: null, booth_fee_amount: 2200 },
  ]);
  // 同じ「鷹尾」でも、ながやま鷹尾は売上の10%、PASIO鷹尾は定額2,200円
  assert.equal(calcBoothFee(boothFeeRuleFor(rules, "ながやま 鷹尾店"), 30000), 3000);
  assert.equal(calcBoothFee(boothFeeRuleFor(rules, "パシオ たかお店"), 30000), 2200);
  // 高城（たかじょう）は鷹尾（たかお）とは別のお店
  assert.equal(calcBoothFee(boothFeeRuleFor(rules, "パシオ 高城店"), 30000), 2200);
});

test("マスタに無い場所・場代が無い場所（マンガ倉庫）は行を作らない", () => {
  const rules = buildBoothFeeRules(MASTER);
