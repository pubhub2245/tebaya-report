...
export type ExpenseRow = {
  ...
  /**
   * 出店料（場代）の行かどうか（2026-09 追加）。
   * 出店場所マスタの決まりから自動で作った行に印を付ける。
   * 場代はレシートが出ないので、この行はレシートの催促の対象にしない。
   */
  booth_fee?: boolean | null;
  /** 自動で入れた金額を人が書き換えたか。true のあいだは自動で上書きしない */
  booth_fee_edited?: boolean | null;
};
