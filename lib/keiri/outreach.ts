 ...
 *   お支払いの方法そのものは 2026-09-25 に **銀行振込** で決まり、本番にも出ています
 *   （司令室 kp181・lib/keiri/payment.ts が唯一の正）。
 *   それでも じゅんが返すのは2つのままにします。お振込先をお伝えする文は
 *   こちら側で1通にまとめて用意する決まりなので（司令室 meta/keiri-reply-templates の④）、
 *   じゅんがその場で口座を書くと、あとから出すご案内と食い違うためです。
 *   下の OUTREACH_REPLY_WARNING（古い値段のリンクを貼らない）は、いまも有効です。
 */
